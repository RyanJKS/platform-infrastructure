...
This hub belongs to the dedicated `PROD-HUB` subscription under `prod/eus2/`.
It is intended to serve `prod` spokes across workload subscriptions such as
`PROD-JKS`. Development and production use separate hub subscriptions.
The subscription ID is unset in `PROD-HUB/subscription.hcl`; set the real ID
